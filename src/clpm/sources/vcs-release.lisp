;;;; Project releases taken from source control. A VCS release requires a repo
;;;; to be checked out before much can be done with it.
;;;;
...
(uiop:define-package #:clpm/sources/vcs-release
    (:use #:cl
          #:alexandria
          #:anaphora
          #:clpm/archives
...
(in-package #:clpm/sources/vcs-release)
    ...
    (asdf::collect-sub*directories-asd-files
     ...
     :collect (lambda (x)
                (let* ((namestring (enough-namestring x (release-lib-pathname release)))
                       ...
                       ;;(system (source-ensure-system (release-source release) primary-system-name))
                       )
