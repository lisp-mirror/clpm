...
(defun copy-context (context)
  (make-instance 'context
                 ...
                 :releases (copy-alist (context-releases context))
        ...
        (pprint-logical-block (stream (context-releases context))
          ...
          (loop
            (let ((release-and-reverse-deps (pprint-pop)))
              (serialize-context-release release-and-reverse-deps
                                         (remove-if-not (lambda (x)
                                                          (eql x (car release-and-reverse-deps)))
...
(defmethod extract-why ((why clpm-system-release))
  (let* ((release (system-release-release why))
         (project (release-project release))
         (system (system-release-system why)))
    (list (system-name system) :project (project-name project))))
...
(defun serialize-context-release (release-and-reverse-deps system-releases stream)
  (destructuring-bind (release . reverse-deps) release-and-reverse-deps
    (let ((project (release-project release)))
      (pprint-logical-block (stream nil :prefix "(" :suffix ")")
        ...
        (write-char #\Space stream)
        (pprint-newline :mandatory stream)
        (pprint-logical-block (stream reverse-deps :prefix "(" :suffix ")")
          (pprint-exit-if-list-exhausted)
          (loop
